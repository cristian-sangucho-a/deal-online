...
export const createProduct = async (req, res) => {
    try {
        ...
        const { product, auction } = await AuctionService.createProduct(req.user.user_id, {
            name,
            description,
            starting_price,
            image_url,
...
export const updateProduct = async (req, res) => {
    try {
        ...
        const product = await AuctionService.updateProduct(req.user.user_id, id, { name, description, image_url });
...
export const placeBid = async (req, res) => {
    try {
        ...
        const { bid, auction } = await AuctionService.placeBid(req.user.user_id, auction_id, amount, message);
...
export const closeAuction = async (req, res) => {
    try {
        ...
        const { auction, winningBid } = await AuctionService.closeAuction(auction_id);
